...
class MostFavoriteScreenViewModel(
    private val getMostFavoriteUseCase: GetMostFavoriteUseCase,
) : ViewModel() {
    ...
    init {
        getMostFavorite(forceReload = false)
    }

    fun getMostFavorite(forceReload: Boolean = false) {
        ...
        viewModelScope.launch {
            ...
            try {
                val (newWallpapers, loadFinished) = getMostFavoriteUseCase(perPage = currentPerPage, loadedWallpaperIds = loadedWallpaperIds)
